function output = Bundle_GS(P_Name, x_0, Type, options)
...
while  w_opt > options.s_tol && N_F_LS < Max_of_Failures && F_s >= options.f_target   && k < options.max_iter && toc < options.max_time && T~=1
...
% Computing the gradients of sampled points 
...
% Updating variables of the outer iteration
...
%Applying gradient selection strategy in the outer iteration (reducing the index set) 
...
%Beginning of inner iterations
        ...
        while w_opt > options.s_tol && toc < options.max_time
                  ...
                  [s,F_s,G_s,I,f_eval,g_eval]=LS(gamma,eps,x,d,w_opt,P_Name,ff,f_eval,g_eval);
